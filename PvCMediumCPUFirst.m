...
if any(CheckVert == -2)
    for Check = 1:3
        if sum(GameState(:,Check)) == -2 % Check For win
          ...
          set(findobj('tag',TTTPos{LocateZero, Check}),'string',Char4Square(2+PlayerFirst));        
          GameState(LocateZero,Check) = PlayerFirst;
...
elseif any(CheckHort == -2)
    for Check = 1:3
        if sum(GameState(Check,:)) == -2 
          LocateZero = find(GameState(Check,:) == 0);
          set(findobj('tag',TTTPos{Check, LocateZero}),'string',Char4Square(2+PlayerFirst));        
          GameState(Check, LocateZero) = PlayerFirst;
...
elseif CheckDiagTop2Bot == -2    
    if sum(GameState([1 5 9])) == -2
        ...
        if FindZero == 1
          set(findobj('tag',TTTPos{FindZero,FindZero}),'string',Char4Square(2+PlayerFirst));        
          GameState(FindZero,FindZero) = PlayerFirst;
...
elseif CheckDiagBot2Top == -2    
    if sum(GameState([3 5 7])) == -2
        ...
        if FindZero == 1
          set(findobj('tag',TTTPos{3,1}),'string',Char4Square(2+PlayerFirst));        
          GameState(3,1) = PlayerFirst;
          set(findobj('tag',TTTPos{3,1}),'enable','inactive');        
          Stop = 2;
        elseif FindZero == 2
          set(findobj('tag',TTTPos{2,2}),'string',Char4Square(2+PlayerFirst));        
          GameState(2,2) = PlayerFirst;
          set(findobj('tag',TTTPos{2,2}),'enable','inactive');        
          Stop = 2;
        elseif FindZero == 3
            set(findobj('tag',TTTPos{1,3}),'string',Char4Square(2+PlayerFirst));        
          GameState(1,3) = PlayerFirst;
